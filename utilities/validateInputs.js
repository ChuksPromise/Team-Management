 ...
 * @param {array} range - Required if validating range values (e.g., [0,3] min range 0 & max range 3).
 * @returns {boolean} - Returns true if the input is valid, otherwise false.
 */
function validateInput(input, confirmInput = null, ageRange = [18, 100]) {
  const name = input.name.toLowerCase();
  const value = input.value.trim();

  let isValid = true;
  let errorMessage = "";
  let confirmInputMessage = "";
  ...
  switch (name) {
    ...
    case "name":
      if (!patterns.name.test(value)) {
        isValid = false;
        errorMessage =
          "Name should contain 2-50 characters with letters, spaces, or hyphens.";
      }
      break;
    ...
    case "email":
      ...
      if (confirmInput && value !== confirmInput.value) {
        isValid = false;
        confirmInputMessage = "Emails do not match.";
      }
      break;
    ...
    case "password":
      ...
      if (confirmInput && value !== confirmInput.value) {
        isValid = false;
        confirmInputMessage = "Passwords do not match.";
      }
      break;
    ...
    case "age":
      const isWithinRange = patterns.range(...ageRange)(value);
      if (!isWithinRange) {
        isValid = false;
  ...
  if (confirmInput) {
    return { isValid, errorMessage, confirmInputMessage };
  }
